...
/**
 * TeamGenerator
 * - Formulario con todos los jugadores para darlos de alta (checkbox).
 * - Genera 2 equipos aleatorios con:
 *   * 1 portero por equipo
 *   * sumas de "value" lo más parecidas posible (heurística con búsqueda de pareja de porteros + greedy).
 */

function normalizePlayers(mod) {
  const m = mod?.default ?? mod;
  let base = [];
  if (Array.isArray(m?.players)) base = m.players;
  else if (Array.isArray(m)) base = m;
  else {
...
function isGK(p) {
  // Portero en mayúsculas en el dummy, admitimos variantes
...
function shuffle(arr, seed = Math.random()) {
  // Fisher–Yates con seed simple para re-roll pseudoaleatorio estable si se quiere
  const a = arr.slice();
  let s = seed;
  for (let i = a.length - 1; i > 0; i--) {
    // LCG muy simple para no traer librerías
...
/**
 * Dada una pareja de porteros fija, reparte el resto (no porteros) con un greedy
 * descendente por valor hacia el equipo con menor suma actual.
 */
function assignFieldPlayersBalanced(gkA, gkB, fieldPlayers) {
  ...
  // Ordenamos por valor desc (y con un pequeño shuffle previo para variedad)
...
/**
 * Busca la mejor pareja de porteros que minimice el diff tras asignar jugadores de campo.
 * Si hay muchas combinaciones, igualmente suelen ser pocas (normal en pachangas).
 */
function buildBestTeams(activePlayers) {
  ...
  if (gks.length < 2) {
    return { error: "Se necesitan al menos 2 porteros dados de alta para generar equipos." };
  }
  if (activePlayers.length < 2) {
    return { error: "Se necesitan al menos 2 jugadores dados de alta." };
  }

  // Probamos todas las parejas de porteros
  let best = null;
...
export default function TeamGenerator() {
  ...
  const [active, setActive] = useState(() => new Set()); // ids activados
  const [result, setResult] = useState(null); // {teamA, teamB, diff} | {error}

  useEffect(() => {
    const list = normalizePlayers(dummy);
    setPlayers(list);
    // por defecto: nadie activo; puedes cambiar a todos activos si prefieres:
    // setActive(new Set(list.map(p => p.id)));
  }, []);
  ...
  const setAll = (checked) => {
    setResult(null);
    if (checked) setActive(new Set(players.map((p) => p.id)));
  ...
  const reroll = () => {
    // Forzamos re-generar variando el orden (el algoritmo interno ya mete aleatoriedad)
      ...
      <div className="relative">
        ...
        <div className="mx-auto max-w-6xl px-4 pt-8 pb-2">
          ...
          <p className="mt-1 text-sm text-white/70">
            Da de alta jugadores y genera 2 equipos equilibrados por valor, con 1 portero en cada equipo.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleGenerate} className="mx-auto max-w-6xl px-4 pb-10 pt-4">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex gap-2">
            <button
              type="submit"
              className="rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-2 text-sm ring-1 ring-emerald-400/30 hover:bg-emerald-500/20"
              disabled={!anyChecked}
              title={!anyChecked ? "Activa al menos 2 jugadores" : "Generar equipos"}
            >
              Generar equipos
            </button>
            <button
              ...
              title="Volver a generar manteniendo la misma selección"
            >
              Volver a generar
            </button>
          </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {players.map((p) => {
            ...
            const tag =
              isGK(p) ? "Portero" : "Jugador";
            return (
              <label
                key={p.id}
                className={[
                  "flex items-center justify-between gap-3 rounded-2xl border p-3",
                  checked ? "border-emerald-400/30 bg-emerald-500/10" : "border-white/10 bg-white/5",
                ].join(" ")}
              >
                ...
                <div className="text-xs rounded-md px-2 py-0.5 ring-1 ring-white/10 bg-white/5 text-white/70">
                  {isGK(p) ? "🧤 GK" : "⚽ CAMPO"}
                </div>
              </label>
            );
          ...
          {result && !result.error && (
            <>
              ...
              <div className="md:col-span-2 text-sm text-white/70">
                Diferencia de valor:{" "}
                <span className="font-semibold text-white">{result.diff}</span>
              </div>
            </>
          )}
        </div>
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="text-sm text-white/80">
          Total valor: <span className="font-mono font-bold">{total}</span>
        </div>
      </div>
      {gk && (
        ...
        {(team || []).map((p) => (
          ...
          >
            <span className="truncate">
              {p.name} {isGK(p) ? <span className="text-white/60">(GK)</span> : null}
            </span>
